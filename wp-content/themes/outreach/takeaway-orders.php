<?php
	...
	$qm=mysql_query("select id, menu_id from reservation_detail where menu_id='".$menu_id."' and reservation_id = '".$reservation_id."'and lunchmeny=0") or die(mysql_error());
	if(mysql_num_rows($qm) > 0){
		...
		if($lunchmeny == 0){ // 0 is for takeaway
			...
			if($lunchmeny==2){ // 2 is for breakfast
				...
				if($tillval_count>0){
					mysql_query("insert into reservation_detail(reservation_id,menu_id,quantity,price,status,lunchmeny,temp_tillval) values('".$reservation_id."','".$menu_id."','".$quan."','".$price."',0,2,1)") or die(mysql_error());
				}
				else{
