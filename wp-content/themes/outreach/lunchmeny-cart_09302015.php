...
<style>
	...
	.lunchmenycart-detail{
		background:#1E3556 url(images/cart-bg.png) repeat-x;
		border:0;
		border-radius:4px;
		padding:12px;
	...
	<div class="forcatering">
        <div class="cartbox">
        	<table align="center" class="rwd-table">
                ...
                <?php
                	$qm=mysql_query("select ml.name as menu, rd.notes as notes, rd.price as price, rd.quantity as quantity, rd.id as id, ml.image as img, ml.id as menu_id, rd.lunchmeny as lunchmeny, rd.dagenslunch as dagens from reservation_detail as rd, menu_lunch_items as ml where ml.id=rd.menu_id and ml.deleted=0 and rd.reservation_id='".$reservation_id."' and rd.lunchmeny=1");
				...
				?>
                	<tr class="<?php echo $theclass.' '.$pimg; ?> menudetails menudetail-<?php echo $r['id'];?>">
                    	...
                    	<td data-th="Rätt:" valign="top">
							<?php 
								if($r['dagens']==0){
									echo strip_tags($r['menu']); 
								}
								else{
									echo 'Dagens Lunch';
								}
           ...
           <div style="font-weight:bold; background:#ddd; padding: 20px; text-align:right;">
                <span>Slutsumma : </span>
                <span align="left" class="thetotal"><?php echo '<label id="sumtotal" alt="'.number_format($subtotal,2).'">'.number_format($subtotal,0).'</label> '.getCurrentCurrency(); ?></span>
           </div>
           
           <div>
               <span style="padding:0 !important;"><a href="javascript:void(0)" class="open-lunchmeny-cart" data-rel="<?php echo $subtotal; ?>">Fortsätt</a></span>
            </div>
...
<script type="text/javascript">
	...
	$(function(){
		...
		var checker = Number($('.lmcart-inner .thetotal label').html());
		if(checker<=0){
			$('.fade, .lunchmenycart-detail, .lunchmenycart').fadeOut();
		...
		$('.close-cart').click(function(){
			$('.fade, .lunchmenycart-detail').fadeOut();
		});
		...
		$('.quan').focusin(function(){
			...
			if(quan<1){
				...
				$.ajax({
						url: "<?php echo $theurl; ?>/lunchmeny-cart.php",
						type: 'POST',
						async: false,
						...
						success: function(value){
							...
							if(opt_cookie === null || opt_cookie === "" || opt_cookie === "null" || opt_cookie === 'undefined'){
								...
								for (i = 0; i < cart_opt.length; i++) { 
									...
									$('.open-lunchmeny-cart').attr('data-rel',cur_total+optval);
								}
							}*/
						}
				});	
		...
		$('.open-lunchmeny-cart').click(function(){
			...
			var total = $(this).attr('data-rel');
			  $.ajax({
				...
				type: 'POST',
				async: false,
				data: 'total='+encodeURIComponent(total)+'&tablename='+encodeURIComponent('lunchmeny_settings'),
				success: function(value){
					
					$('.lunchmenycart-detail').css({'display':'none'});
   ...
   $('.opt-item input[type=checkbox]').click(function(){
		...
		if (this.checked) {
			$('#'+st_tar).html(cur_stotal+optval);
			$('#sumtotal').html(cur_total+optval);
			$('.open-lunchmeny-cart').attr('data-rel',formatNumber(cur_total+optval));
		}else{
			$('#'+st_tar).html(cur_stotal-optval);
			$('#sumtotal').html(cur_total-optval);
			$('.open-lunchmeny-cart').attr('data-rel',formatNumber(cur_total-optval));
...
<script type="text/javascript">
$(function(){	
		$(".lmcart-inner").css('overflow' ,'auto');
		var cart_scroller = $(".lmcart-inner");
