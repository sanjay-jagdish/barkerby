<?php
	...
	define("PASSWORD",'root');
        ...
        /*define("USER",'limone_wrdp1');
	define("PASSWORD",'owwFkNjB5hee');*/
	//define('DB_PASSWORD', 'owwFkNjB5hee');
	...
	function getPrivateKey(){
		$q=mysql_query("select var_value from settings where var_name = 'private_key'") or die(mysql_error());
		$r=mysql_fetch_assoc($q);
		
		return $r['var_value'];
	}
	
	function getOrderId($uniq){
		$q=mysql_query("select id from reservation where uniqueid='".$uniq."' and deleted=0") or die(mysql_error());
		$r=mysql_fetch_assoc($q);
