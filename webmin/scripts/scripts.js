...
function addMenu(){
	...
	var takeaway_price=jQuery('.takeaway-price').val();	
	var discount_unit = 'fix';
	
	var takeaway_tillval=jQuery('.takeaway-tillval').val();
	...
	if(name!='' & des!='' & price!='' & menutype!=''){
		...
		jQuery.ajax({
				 ...
				 type: 'POST',
				 data: 'cat='+encodeURIComponent(cat)+'&cattype='+encodeURIComponent(cat_type)+'&name='+encodeURIComponent(name)+'&des='+encodeURIComponent(des)+'&price='+encodeURIComponent(price)+'&img='+encodeURIComponent(img)+'&currency='+encodeURIComponent(currency)+'&featured='+encodeURIComponent(featured)+'&menutype='+encodeURIComponent(menutype)+'&takeaway_price='+encodeURIComponent(takeaway_price)+'&discount_unit='+encodeURIComponent(discount_unit)+'&takeaway_tillval='+encodeURIComponent(takeaway_tillval),
				 success: function(value){
					 if(value!='Invalid'){
...
function editMenu(){
	...
	var isvisalrat = jQuery('.fromall-holder').val();
	...
	if(name!='' & des!='' & price!='' & menutype!=''){
		...
		jQuery.ajax({
				 ...
				 type: 'POST',
				 data: 'cat='+encodeURIComponent(cat)+'&cattype='+encodeURIComponent(cat_type)+'&name='+encodeURIComponent(name)+'&des='+encodeURIComponent(des)+'&price='+encodeURIComponent(price)+'&img='+encodeURIComponent(img)+'&currency='+encodeURIComponent(currency)+'&id='+encodeURIComponent(id)+'&featured='+encodeURIComponent(featured)+'&menutype='+encodeURIComponent(menutype)+'&takeaway_price='+encodeURIComponent(takeaway_price)+'&discount_unit='+encodeURIComponent(discount_unit)+'&takeaway_tillval='+encodeURIComponent(takeaway_tillval),
				 success: function(value){
					 if(value!='Invalid'){
						editOptions(value, nav, cat_type, cat, isvisalrat);
						//setTimeout("window.location.reload();",2000); 
					 }
					 else{
...
function addOptions(id, nav){
	...
	var count = 0;
	$('.mopts').each(function(){
		var grpname = $(this).children('.tillvalname').val();
		var typ = $(this).children('.takeaway-tillval').val();
		
		count++;
		
		if(grpname!=''){
			$(this).find('.theopt').each(function() {
				var optname = $(this).children('.optname').val();    
				var optprice = $(this).children('.optprice').val();    
				
				if(grpname!='' && optname!=''){
					var indicator=grpname+'***'+count;
					if(grp_arr.indexOf(indicator+'^^^'+typ)==-1){
						grp_arr.push(indicator+'^^^'+typ);
					}
					opt_arr.push(indicator+'^*^'+optname+'^*^'+optprice);
				}
	...
	jQuery.ajax({
		 ...
		 type: 'POST',
		 data: {option:opt_arr,id:id, group:grp_arr},
		 success: function(value){
...
function editOptions(id, nav, edited_menucattype, edited_menucatid, isvisalrat){
	...
	$('.mopts').each(function(){
		var grpname = $(this).children('.tillvalname').val();
		var typ = $(this).children('.takeaway-tillval').val();
		var grporder = $(this).find('.grporder').val();
		...
		if(grpname!=''){
			$(this).find('.theopt').each(function() {
				var optname = $(this).children('.optname').val();    
				var optprice = $(this).children('.optprice').val(); 
				var optorder = $(this).find('.optorder').val();   
				
				if(grpname!='' && optname!=''){
					var indicator=grpname+'***'+count;
					if(grp_arr.indexOf(indicator+'^^^'+typ+'^^^'+grporder)==-1){
						grp_arr.push(indicator+'^^^'+typ+'^^^'+grporder);
					}
					opt_arr.push(indicator+'^*^'+optname+'^*^'+optprice+'^*^'+optorder);
				}
	...
	jQuery.ajax({
		 ...
		 success: function(value){
			 ...
			 if(nav=='takeaway'){
				 if(isvisalrat=='false'){
			 	setTimeout('window.location="?page=takeaway-menu&parent=takeaway&editedmenucattype='+edited_menucattype+'&editedmenucatid='+edited_menucatid+'&tab=huvudmeny"',2000);
				 }else{
				setTimeout('window.location="?page=takeaway-menu&parent=takeaway&fromall='+isvisalrat+'&tab=huvudmeny"',2000); 
				}
			 }
			 else{
				if(isvisalrat=='false'){
			 	setTimeout('window.location="?page=menu&parent=menu&editedmenucattype='+edited_menucattype+'&editedmenucatid='+edited_menucatid+'&tab=huvudmeny"',2000);
				 }else{
				setTimeout('window.location="?page=menu&parent=menu&fromall='+isvisalrat+'&tab=huvudmeny"',2000); 
				}
			}
		 }
	});
		...
		jQuery.ajax({
			 ...
			 success: function(value){
				 ...
				 if(count!=0){
					 ...
					 jQuery(".orders-li").prepend("<div id='notif'>"+count+"</div>");
