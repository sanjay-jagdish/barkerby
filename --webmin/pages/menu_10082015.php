            ...
            <?php
					...
					if(mysql_num_rows($qcategory)>0){
						while($rcat = mysql_fetch_assoc($qcategory) ){
							...
							$qc=mysql_query("select c.name, m.name, m.description, m.image, m.price, m.id, cu.shortname, m.featured, m.type, c.id, m.discount, m.discount_unit from category as c, menu as m, currency as cu where c.id=m.cat_id and m.deleted=0 and cu.id=m.currency_id and m.cat_id = $rcat[id] and m.type<>'2' order by m.order asc") or die(mysql_error());
									...
									<tbody>
										...
										?>
											<tr class="gradeX gradeX-<?php echo $r[5].' '.$theclass;?>" align="center">
												...
												<td><a href="?page=menu&subpage=edit-menu&id=<?php echo $r[5]; ?>&nav=takeaway" class="edit-menu" title="Redigera rätt"><img src="images/edit.png" alt="Redigera rätt"></a> <?php if($_SESSION['login']['type']<3){ ?><a href="javascript:void" class="delete-menu" title="Radera rätt" data-rel="<?php echo $r[5]; ?>"><img src="images/delete.png" alt="Radera rätt"></a><?php } ?></td>
											</tr>
										<?php		
											}
										?>
               ...
               <table cellpadding="0" cellspacing="0" border="0" class="display" id="datatable-<?php echo $rscat['id']?>">
                    ...
                    <tbody>
                        <?php
							$q=mysql_query("select c.name, m.name, m.description, m.image, m.price, m.id, cu.shortname, m.featured, m.type, c.id, m.discount, m.discount_unit from sub_category as c, menu as m, currency as cu where c.id=m.sub_category_id and m.deleted=0 and cu.id=m.currency_id and m.sub_category_id = $rscat[id] and m.type<>'2' order by m.order asc") or die(mysql_error());
