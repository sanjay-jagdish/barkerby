...
<?php //need actions
	...
	while($r=mysql_fetch_assoc($qq)){
	 ...
	 ?>
			...
			<div class="<?php echo 'bong grp-'.$rowcount;?> <?php echo ($rst['description']=="Cancel")? 'order-cancel ':''; echo ($bongs==1) ? 'on-progress ':'pending '; echo ($r['approve']!=8) ? ' has-time ': ' no-time '; echo ($r['viewed']==0) ? ' new-blink ' : 'viewed'; ?> gradeX gradeX-<?php echo $r['rid'];?>">
				...
				<div class="pointer" onclick="showDetail('<?php echo $r['rid'];?>','<?php echo $r['rtid'];?>')">
					<div class="pickuptime" >
						<?php
							if($countdays==0){
								$prefx='Idag'.', <span>'.$r['asaptime'].'</span>'; 
							}else if($countdays==1){
								$prefx='Imorgon'.', <span>'.$r['asaptime'].'</span>'; 
							}else if($countdays<0){
								if($countdays==-1){
									$prefx='Igår'.', <span>'.$r['asaptime'].'</span>'; 
								}
								else{
								...
								$prefx='Om '.$countdays.' dagar'.', <span>'.$r['asaptime'].'</span>'; 
							}
							...
							if($r['approve']!=8){
								...
								if($r['asap']==0){
									echo '<span>Annan Tid</span>'; 
								}
								else{
									echo '<span>Snarast</span>';
								}
