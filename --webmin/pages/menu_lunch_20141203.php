...
<div style="background-color:#FFC; padding:10px 15px; overflow:hidden; display:none;"> OBS! Gäller hela vecka <?php echo $week_num; ?> (mån-fre)
    ...
    <div style="float:right; width: 170px; text-align:center;">
        ...
        <input type="button" value="Uppdatera vecka <?php echo $week_num; ?>" class="btn" id="<?php echo $save_button_id; ?>" style="padding: 15px 25px; margin-top: 40px; font-family: 'Roboto';" />
   		<br />
   		<?php
		}
		?>
		...
		<div class="menu_item" id="existing_course_<?php echo $courses_res['id']; ?>">
            ...
            <div class="menu_item_opt">
                ...
                <div class="lmtakeaway">
                	<ul>
                    	<li><input type="checkbox" class="lmcbox" id="lmcbox<?php echo $courses_res['id']; ?>" data-rel="lmlabel<?php echo $courses_res['id']; ?>" data-id="lmtapris<?php echo $courses_res['id']; ?>" onclick="lmfield('lmcbox<?php echo $courses_res['id']; ?>')"><label for="lmcbox<?php echo $courses_res['id']; ?>">Take away</label></li>
                        <li class="lmlabels lmlabel<?php echo $courses_res['id']; ?>">Take away Pris: <input type="text" class="lmtapris<?php echo $courses_res['id']; ?>"></li>
                    </ul>
                </div>
...
$('#add_course').on('click', function (e) {
	add_count+=1;
	$( "#menu_items_additional" ).append('<div class="menu_item" id="additional_course_'+add_count+'"><div class="menu_item_actions"><img src="images/delete.png" alt="Radera" onClick="remove_additional_course('+add_count+')"><br /></div><div class="menu_item_desc">Rätt: <div class="additional_course_name" id="additional_course_name_'+add_count+'"  contenteditable="true"></div><br>Beskrivning:<br><div class="additional_course_desc" id="additionalTextarea_'+add_count+'" contenteditable="true"></div></div><div class="menu_item_opt">bcaPris: <input type="text" class="additional_course_price" id="additional_course_price_'+add_count+'" /> <?php echo $currency_res['shortname']; ?><div class="lmtakeaway"><ul><li><input type="checkbox" class="lmcbox" id="almcbox'+add_count+'" data-rel="almlabel'+add_count+'" data-id="almtapris'+add_count+'" onclick="lmfield(\'almcbox'+add_count+'\')"><label for="almcbox'+add_count+'">Take away</label></li><li class="lmlabels almlabel'+add_count+'">Take away Pris: <input type="text" class="almtapris'+add_count+'"></li></ul></div><br /><input type="checkbox" id="allweek_additional_'+add_count+'">Gäller alla veckor.</div><span class="link_button paste_from_clip" onclick="ccbd('+add_count+')" id="ccbd_'+add_count+'">Klistra in</span></div>');
...
$('#lunch_meny_update_all').on('click', function (e) {
	...
	$(".course_desc_existing").each(function(){
		...
		if(price>0){
			...
			var val_desc_chk = $('#course_name_existing_'+id_data[1]).text().trim();
			var val_name_chk = $('#e_'+id_data[1]).text().trim();
			...
			if(parameter_check==2){ 
				errors++; 
				if(val_desc_chk==''){ $('#e_'+id_data[1]).addClass('warning'); }
				if(val_name_chk==''){ $('#course_name_existing_'+id_data[1]).addClass('warning'); }			
	...
	$(".additional_course_desc").each(function(){
		...
		if(price>0){
			...
			if(parameter_check==2){ 
				...
				if(val_name_chk==''){ $('#additional_course_name_'+id_data[1]).addClass('warning'); }			
			}
...
function ccbd(id){
	...
	$.post( "actions/menu_lunch_paste_clipboard.php", { id:id })
		.done(function( data ){
			...
			if(info[0]=='' && info[1]=='' && info[2]==''){ alert('Clipboard is empty.'); }
