...
<style>
.menu_main_description, .course_desc,  .course_desc_existing, .additional_course_desc{ width: 735px; height: 54px; }
...
<div style="background-color:#FFC; padding:10px; overflow:hidden;">
    ...
    <div style="float:right; width: 170px; text-align:center;">
        ...
        <input type="button" value="Spara allt för vecka <?php echo $week_num; ?>" class="btn" id="<?php echo $save_button_id; ?>" style="padding: 15px 25px; margin-top: 40px;" />
   		<br />
        ...
        <table>
            <tr>
                <td align="left">Text ovanför menyn:</td>
          ...
          <tr>
            <td align="left"><span style="clear:both; clear:both;">Text nedanför menyn:</span></td>
    ...
    <br /><br />
    <span style="font-size: 20px; padding-left:10px;">Courses:</span>
    <br /><br /><br />
...
$('#add_course_<?php echo strtolower(date('D', $the_day)); ?>').on('click', function (e) {
	add_count+=1;
	$( "#menu_items_additional_<?php echo strtolower(date('D', $the_day)); ?>" ).append('<div class="menu_item" id="additional_course_<?php echo strtolower(date('D', $the_day)); ?>_'+add_count+'"><div class="menu_item_actions"><img src="images/delete.png" alt="Radera" onClick="remove_additional_course_<?php echo strtolower(date('D', $the_day)); ?>('+add_count+')"><br /></div><div class="menu_item_desc"><textarea class="course_additional_day" data-rel="<?php echo strtolower(date('D', $the_day)); ?>" id="additionalTextarea_<?php echo strtolower(date('D', $the_day)); ?>_'+add_count+'" style="width: 795px; height: 50px;"></textarea></div>Pris:<input type="text" class="additional_course_price_<?php echo strtolower(date('D', $the_day)); ?>" id="additional_course_price_<?php echo strtolower(date('D', $the_day)); ?>_'+add_count+'" /> <?php echo $currency_res['shortname']; ?><br /><br /><input type="checkbox" id="all_<?php echo strtolower(date('D', $the_day)); ?>_additional_'+add_count+'">Gäller alla <?php echo dayName(date('D', $the_day)); ?>dagar.</div></div>');
...
$('#lunch_meny_save_all').on('click', function (e) {
	...
	if(text_over_menu==''){
		errors++;
		$('#text_over_menu').addClass('warning');
		errorsArray.push('text_over_menu');
	}else{
		$('#text_over_menu').removeClass('warning');
	}
	...
	if(menu_main_description==''){
		errors++;
		$('.menu_main_description').addClass('warning');
		errorsArray.push('menu_main_description');
	}else{
		$('.menu_main_description').removeClass('warning');
	}
	...
	$(".additional_course_desc").each(function(){
		...
		if(price>0){
			...
			$('#additional_course_price_'+id_data[1]).addClass('warning');
			$(this).addClass('warning');
			errorsArray.push('additional_course_price_'+id_data[1]);
		}
		
		if(val_desc==''){
			errors++;
			$(this).addClass('warning');
			errorsArray.push('val_desc');
		}else{
			$(this).removeClass('warning');			
			courses = courses+separator+val_desc; 		
		}	
	...
	$(".course_additional_day").each(function(){
		...
		course_desc = $('#additionalTextarea_'+day+'_'+id_arr[2]).val();
		course_prc = $('#additional_course_price_'+day+'_'+id_arr[2]).val();
		allin = $('#all_'+day+'_additional_'+id_arr[2]).val();
	...
	if(text_after_menu==''){
		errors++;
		$('#text_after_menu').addClass('warning');
		errorsArray.push('text_after_menu');
	}else{
		$('#text_after_menu').removeClass('warning');
	}
...
$('#pdf_export').on('click', function (e) {
		...
		var data_rel = $(this).attr('data-rel');
		var data_array = data_rel.split('-');
		window.open('http://<?php echo $_SERVER['HTTP_HOST']; ?>/webmin/pages/preview_lunch_meny.php?year='+data_array[0]+'&week='+data_array[1]+'&pdf=1', '_blank');
