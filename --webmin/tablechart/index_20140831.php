...
<?php
$table_reservations_sql = "SELECT r.id, r.date, r.time, ADDTIME(TIME(r.time), SEC_TO_TIME(r.duration*60)) AS end, r.duration, 
							a.fname, a.lname, 
							t.table_name, rt.table_detail_id, r.number_people, r.note, r.status  
							FROM reservation r
							...
							WHERE r.date='".date('m/d/Y',strtotime($date_selected))."' AND r.approve=2
							AND t.restaurant_detail_id=".$res_det['id']." AND rt.deleted=0 
...
?>
	<tr class="reserved" data-rel="<?php echo $table_reservations['id']; ?>">
        ...
        <td class="<?php echo $status_class; ?>"><?php echo $table_reservations['number_people']; ?></td>
        <td class="<?php echo $status_class; ?>" style="text-align:center;"><?php echo $table_reservations['table_name']; ?></td>
        ...
        <td class="<?php echo $status_class; ?>" style="text-align:left;"><?php echo $table_reservations['note']; ?></td>
    </tr>
<?php
	}
	echo '</tbody>';
        	...
        	<thead>
				...
				while($time_h<=$end_h){
					if(date('i',$time_h)=='00'){
						...
						$class_th = '';
                    ?>
				  <th class="<?php echo $class_th; ?>" colspan="<?php echo $hr_span; ?>">
					<?php 
					echo $time_txt_h;
					?>
                    </th>
				...
				while($time<=$end){
					...
					?>
                    <th style="width:70px;" class="<?php echo $tdClass; ?>" bgcolor="<?php echo $rowColor; ?>">
					<?php 
					echo $time_txt;
					?>
                    </th>
